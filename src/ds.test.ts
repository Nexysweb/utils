...
test("group by with type", () => {
  ...
  const r: { [k: string]: { a: string; v: number }[] } = DSUtils.groupBy(
    value,
    "a"
  );
...
test("unique", () => {
  ...
  expect(DSUtils.unique(r, "ModuleId")).toEqual([
    { ModuleId: 1 },
    { ModuleId: 2 },
    { ModuleId: 3 },
  ]);
...
test("keep props", () => {
  ...
  expect(DSUtils.keepProps(obj, ["test", "hello"])).toEqual({
    test: "me",
    hello: "world",
  });
});
...
test("updateObject, 2d", () => {
  ...
  expect(DSUtils.updateObject(form, newObj)).toEqual({
    myname: { first: "myvalue" },
  });
});
...
test("updateObject, 2d, with already existing", () => {
  ...
  expect(DSUtils.updateObject(form, newObj)).toEqual({
    myname: { first: "myvalue", second: "mysecond" },
  });
});
