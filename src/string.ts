...
export const formatPhone = (p: string): string => {
  ...
  if (usPrefix === "001" && p.length === 13) {
    return (
      "+1 (" + p.substr(3, 3) + ") " + p.substr(6, 3) + "-" + p.substr(9, 4)
    );
  }
...
export const snakeToCamelCase = (str: string): string =>
  str.replace(/(\_\w)/g, (c) => c[1].toUpperCase());
...
export const lowerFirst = (s: string): string =>
  typeof s === "string" ? s.charAt(0).toLowerCase() + s.substr(1) : s;

/**
 *
 * @param s given a size in b, format to kb and MB
 */
